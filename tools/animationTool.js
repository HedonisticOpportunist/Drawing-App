    ...
    if (mouseIsPressed && drawingProperties.sliderValue != null) {
      ...
      if (this.startMouseX == -1) {
        ...
        if (eventConditions.drawRubikCondition != false) {
          ...
          this.sprite.velocity.x = random(
            -1 * drawingProperties.sliderValue,
            drawingProperties.sliderValue - 10
          );
          this.sprite.velocity.y = random(
            -1 * drawingProperties.sliderValue,
            drawingProperties.sliderValue - 10
          );
          drawSprites();
        ...
        if (eventConditions.drawCatsCondition != false) {
          ...
          this.otherCat = createSprite(
            this.startMouseX - 100,
            this.startMouseY - 100,
            50,
            100
          );
          this.cat.addAnimation(
            "running",
            imageProperties.non_sequence_animations[0]
          );
          this.otherCat.addAnimation(
            "resting",
            imageProperties.non_sequence_animations[2]
          );
        ...
        if (eventConditions.drawGhostCondition != false) {
          ...
          this.ghost.addAnimation(
            "haunting",
            imageProperties.non_sequence_animations[1]
          );
        ...
        if (eventConditions.drawKittensCondition != false) {
          animation(
            imageProperties.sequenceAnimation,
            this.startMouseX,
            this.startMouseY
          );
        }
